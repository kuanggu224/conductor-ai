...
Audit bundle schema `1.2` 会把 Manifest schema、最终状态、`summary.pending_test_scope`、human control action count 和 active human hold 复制到 bundle summary；验证 audit bundle 时会重新读取 Manifest 交叉校验这些字段，使返工后的复验范围和人工接管状态在最终交付审计包中也可追踪。
...
`status-all` 会扫描同一个 workspace 下的全部项目，输出 `active_project_ids`、`active_actions`、每个项目的 hold reason 和可复制 operator commands，便于长周期调度器或人工 operator 先确认哪些项目正处于暂停或审批 gate。`--active-only` 可只输出被 hold 的项目，`--fail-on-active` 会在存在 active hold 时返回退出码 `3`，`--output` 可把 JSON 写入文件供调度器或看板读取。
...
python -m app.human_control status-all --project-root <workspace-root> --active-only --fail-on-active --output .conductor\human-control\status.json
