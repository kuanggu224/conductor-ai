...
- 全栈前后端需求会映射到 `fullstack_integration` testing checklist，必须输出 `Fullstack frontend API integration verified` 或 `browser fetch /api/items` 等浏览器/API 联调证据，不能只用 API contract tests 代替前端集成证明。
