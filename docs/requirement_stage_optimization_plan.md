...
- Run Manifest schema 已升级到 `1.36`，包含 `requirement_evaluations`、`summary.requirement_quality_score`、preflight gate 索引、preflight 修复建议、`retry_history`、`scope_contract_results`、`prompt_hash`、`summary.llm_context_windows`、`summary.llm_token_usage`、`summary.llm_cost_estimate`、`resume_cursor` 和运行审计摘要。
