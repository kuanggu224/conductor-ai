...
- 人类随时接管与审批工作流；TL 将 `escalate_project` 判定为 `human_action_required` 时，LeadController 会先创建匹配 approval gate，人工 approve / override 后才继续升级。`human_control status-all` 可跨 workspace 汇总 active holds，并支持 `--active-only`、`--fail-on-active` 与 `--output`，方便长周期 operator 或调度器先看到哪些项目正被暂停或等待审批。

当前 TL 动态规划已经能识别运行时失败、历史角色失败率、开发范围复杂度、带有缺失 testing checklist evidence 的开发返工任务，以及带机器可读 testing checklist 的测试任务。对于开发返工，TL 会追加 `rework_acceptance_guard` tester 席位，专门检查返工是否补齐缺失验收证据和回归风险；对于测试 checklist 证据契约，TL 会追加 `evidence_trace_guard` tester 席位，审计每条 required evidence 是否有可观察证据支撑。对于同时拆出前端和后端并行开发的复杂实现，TL 会追加 `integration_contract_guard` solution_designer 席位，提前复核 API/UI/data 契约、校验边界和交接风险；当 development 阶段 WorkItem、实现类型、验收点或输入基线显示开发范围较宽时，TL 会追加 `implementation_coordination_guard` solution_designer 席位，先审查 write scope、依赖顺序、交接边界和归并风险，避免多 Agent 各自开工后才暴露冲突。
...
Project Report 的 `## Task Center` 和 Replay trace 会直接渲染这些关联/缺失 Artifact id，让只读复盘和人工维护不用再打开原始 Manifest JSON 才能定位修复目标。
...
当前可审计交互证据包括表单提交、localStorage 刷新保留、筛选/搜索、删除、导出下载，以及文件导入/上传样本处理。多功能需求会先生成 `feature_slice_plan`，把 create/list/filter/delete/stats/import/export 等用户可见能力拆成 milestone-based feature slices，并把实现顺序与验证焦点写入 development/testing WorkItem 的 acceptance criteria。文件处理类需求会映射到 `file_import` testing checklist，并要求 `Browser file import processed sample file` 证据；Harness 生成的测试报告会渲染 `Testing Checklist Evidence Contract`，把每条 required evidence term 写入持久化测试 Artifact。`static_web_delivery` 只会在冻结需求要求 import/upload 时生成真实 CSV 导入控件，避免把未请求的导入能力扩进交付范围。`api_mock` run profile 会通过 `api_mock_delivery` 生成 FastAPI mock 服务、pytest API contract tests 和可审计 endpoint/status/payload 输出，补齐 API mock 流的离线端到端案例；`api_sqlite` run profile 会通过 `api_sqlite_delivery` 生成 FastAPI + SQLite 服务、pytest API contract tests 和 `SQLite persistence verified` 数据库持久化证据，作为从 mock API 走向真实后端的下一层离线案例；`fullstack_web` run profile 会通过 `fullstack_web_delivery` 生成 FastAPI 后端、静态前端页面、pytest + Playwright 全栈 contract tests，并输出 `Browser form interaction updated visible state`、`GET /api/items/stats -> status_code=200 response payload=` 和 `Fullstack frontend API integration verified` 等浏览器/API 集成证据。后端/API 类需求会映射到 `api_behavior` testing checklist；明确要求 SQLite/database persistence 的需求还会映射到 `db_persistence` testing checklist。`api_validation` WorkItem 必须在成功输出中暴露 endpoint、status code 或 response payload 等具体接口行为信号，才会记录 `API validation exercised endpoint behavior`，避免 API mock / 后端验收只停留在泛化的 `pytest passed`。当测试失败回流为开发返工时，缺失的 testing checklist evidence 会进入返工 WorkItem 的 acceptance criteria，并出现在 Task Center rework prompt 中；结构化反馈还会携带最近一次测试执行的 `validation_command` 和 `validation_exit_code`，`rework_context.pending_retest_scope` 会暴露修复后应优先复跑的最小测试范围，Project Report 与 Run Manifest summary 也会记录项目级 `pending_test_scope`，外部 worker 可以直接看到需要补齐哪条验收证据以及应该重跑哪条验证命令。
...
`app.diagnostics` 和 `run_project --diagnose` 会暴露 CLI 探测、CLI auth status、LLM server/model/context window、LLM timeout 健康状态、UTF-8 编码就绪度和最近一次 preflight gate。`--probe-cli` 会从轻量探测输出中识别未登录、认证过期、API key 缺失等常见授权失败，并输出 `auth_status=unauthorized`、`auth_error` 和 login/auth 恢复建议；selected CLI 的未授权状态会进入 preflight gate warnings，避免真实执行开始后才失败。`--preflight-llm` 会输出 `failure_category`，区分 auth、quota/rate limit、context length、model not found、timeout、network 和 server error 等 provider 故障，并给出对应恢复建议。`timeout_status=low` 用于提醒长 prompt 风险；`timeout_status=invalid` 会阻断 LLM model probe，并在 warnings 中给出明确修复建议。
