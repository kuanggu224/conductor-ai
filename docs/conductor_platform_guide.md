...
- 人类随时接管与审批工作流；TL 将 `escalate_project` 判定为 `human_action_required` 时，LeadController 会先创建匹配 approval gate，人工 approve / override 后才继续升级。`human_control status-all` 可跨 workspace 汇总 active holds，方便长周期 operator 先看到哪些项目正被暂停或等待审批。
