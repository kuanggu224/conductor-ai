...
def test_engine_writes_run_manifest(tmp_path) -> None:
    ...
    assert "testing_checklist" in payload["workitems"][0]
...
def test_manifest_records_stale_claimed_task_assignments(tmp_path) -> None:
    ...
    assert "missing_artifact_ids" in payload["task_center_audit"][0]
    ...
    assert payload["requirement_coverage_results"] == [
        {
            ...
            "passed": False,
            "required_rules": ["add_item", "persistence", "export_csv", "filter", "delete_item"],
            "covered_rules": ["add_item", "export_csv", "filter", "delete_item"],
