...
def test_design_quality_accepts_actionable_english_design() -> None:
    ...
    ## Goal
    Deliver a api-only study tracker.
    ...
    ## Data And State
    Data fields are id, question, answer, topic, status, and createdAt. State lives in memory and SQLite.

    ## Acceptance And Test Plan
    Test creation, validation error, filter behavior, delete persistence, reload persistence, and CSV export.

    ## Risks And Assumptions
    Risk: SQLite can be cleared. Assumption: single-user local api client use. Open question: exact status vocabulary.
    """
