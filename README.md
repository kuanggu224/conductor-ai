...
CLI/API 在创建外部输出 Artifact 前会先校验 claim 状态、`agent_id` 和 `claim_token`，错误 worker 或 stale token 的归还请求不会留下未引用的外部 Artifact；Task Center audit/maintenance 也会报告历史遗留的 `orphan_external_artifact`，并通过 `related_artifact_ids` 保留可校验引用。缺失输入/输出 Artifact 会进入 `missing_artifact_ids`，便于调度器或 operator 精确修复状态。
