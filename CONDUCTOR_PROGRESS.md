...
- `python -m app.run_project --write-audit-bundle` 可一次性归档 manifest 自检报告和 replay trace，并写出 audit bundle index JSON，索引 manifest、report、verification、replay trace 路径、通过状态和 SHA-256；写出后会立即执行 `verify_audit_bundle` 并把结果放进最终 JSON，若自检失败则 `run_project` 返回 2。
...
- 当前全量测试：`python -m pytest -q`，结果 `421 passed`。
