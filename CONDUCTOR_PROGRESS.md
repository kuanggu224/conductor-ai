...
- Manifest verifier 已覆盖 `files.artifacts` / `files.task_prompts` 与顶层 `artifact_files` / `task_prompt_files` 的存在性和索引一致性 warning。
- 针对测试：`python -m pytest tests\test_replay_verifier.py -q`，结果 `30 passed`。
...
- 当前全量测试：`python -m pytest -q`，结果 `439 passed`。
