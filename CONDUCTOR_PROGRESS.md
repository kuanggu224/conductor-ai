...
- `python -m app.verify_audit_bundle <audit.json>` 可校验 audit bundle index 结构、schema `1.0`、组件文件存在性、verification/replay 摘要状态和 replay trace 身份/pass 标记，并重新执行 manifest verifier 防止 verification 报告过期；支持 `--fail-on-warnings`。
...
- 当前全量测试：`python -m pytest -q`，结果 `414 passed`。
